...
class SignUpController: UIViewController,  UIImagePickerControllerDelegate, UINavigationControllerDelegate{
            ...
            storageRef.putData(uploadData, metadata: nil, completion: { (metadata, err) in
                ...
                storageRef.downloadURL(completion: { (downloadURL, err) in
                    ...
                    Database.database().reference().child("users").updateChildValues(values, withCompletionBlock: { (err, ref) in
                        ...
                        // transfer to MainTabBarController
    ...
    let alreadyHaveAccountButton: UIButton = {
        ...
        // triger handleAlreadyHaveAccount function and jump to login view
        button.addTarget(self, action: #selector(handleAlreadyHaveAccount), for: .touchUpInside)
        return button
    }()
    
    @objc func handleAlreadyHaveAccount() {
        // pop out this SignUp View and return to the sign in view
    ...
    override func viewDidLoad() {
        ...
        setPlusButtonView()
        setupInputFields()
        setupSignInRemindView()

    }
    
    func setupSignInRemindView(){
        view.addSubview(alreadyHaveAccountButton)
        alreadyHaveAccountButton.anchor(top: nil, left: view.leftAnchor, bottom: view.bottomAnchor, right: view.rightAnchor, paddingTop: 0, paddingLeft: 0, paddingBottom: 0, paddingRight: 0, width: 0, height: 50)
    }
    
    func setPlusButtonView(){
...
// set Every View's anchor and size
extension UIView {
